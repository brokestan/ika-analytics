...
interface LockItem {
  duration: number; label: string; percentage: number;
  total_nfts: number; total_ika: number; rate: number;
}
...
export default function LockDistributionChart({ data, loading }: { data: LockItem[]; loading?: boolean }) {
  if (loading) return (
  ...
  return (
    <div className="card p-5 animate-slide-up" style={{ animationDelay: '150ms' }}>
      ...
      {!hasData ? (
        ...
        <div className="space-y-3">
          {data.map((item) => {
            const c = COLORS[item.duration] || COLORS[30];
            return (
              <div key={item.duration} className="bg-white/3 border border-white/5 rounded-xl p-3 hover:border-white/10 transition-colors">
                <div className="flex items-center justify-between mb-2">
                  <span className={clsx('text-sm font-semibold', c.text)}>{item.label}</span>
                  <div className="flex items-center gap-1.5">
                    <TrendingUp className="w-3 h-3 text-ika-muted" />
                    <span className="text-xs text-ika-dim font-mono">{item.rate}x rate</span>
                  </div>
                </div>
                <div className="h-1.5 bg-white/5 rounded-full overflow-hidden mb-2">
                  <div className={clsx('h-full rounded-full transition-all duration-700', c.bar)} style={{ width: `${item.percentage}%` }} />
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className={clsx('text-xs font-mono font-bold px-2 py-0.5 rounded-full', c.badge)}>{item.percentage}%</span>
