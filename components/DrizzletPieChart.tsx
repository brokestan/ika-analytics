    ...
    <div className="flex flex-wrap gap-x-4 gap-y-2 justify-center mt-2">
      {payload?.map((e) => (
        <div key={e.value} className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-ika-dim">{e.value}</span>
        </div>
      ))}
    </div>
  );
...
export default function DrizzletPieChart({ data, loading }: { data: DrizzletDistribution | null; loading?: boolean }) {
  ...
  const chartData = SEGMENTS
    .map((s) => ({ name: s.label, value: Math.max(0, data[s.key]), color: s.color }))
    .filter((d) => d.value > 0);

  const total = chartData.reduce((s, d) => s + d.value, 0);
  if (total === 0) chartData.push({ name: 'No data yet' as any, value: 1, color: '#2A1F3D' as any });
  ...
  return (
    <div className="card p-5 animate-slide-up" style={{ animationDelay: '100ms' }}>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <PieChart>
          <Pie data={chartData} cx="50%" cy="45%" innerRadius={65} outerRadius={100} paddingAngle={3} dataKey="value" strokeWidth={0}>
