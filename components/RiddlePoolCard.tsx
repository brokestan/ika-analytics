...
interface RiddlePoolData {
  pool1: number; pool2: number; pool3: number;
  total: number; fetched_at: string | null;
}

export default function RiddlePoolCard({ data, loading }: { data: RiddlePoolData | null; loading?: boolean }) {
  ...
  const pools = [
    { label: 'Pool 1', value: data.pool1 },
    { label: 'Pool 2', value: data.pool2 },
    { label: 'Pool 3', value: data.pool3 },
  ];
  ...
  return (
    <div className="card p-5 animate-slide-up" style={{ animationDelay: '200ms' }}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {pools.map((pool, i) => (
          <div key={i} className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-3 text-center hover:border-amber-500/30 transition-colors">
            <p className="text-xs text-ika-dim mb-1">{pool.label}</p>
            <p className="font-mono font-bold text-white text-base leading-none">{formatNumber(pool.value, 1)}</p>
