...
export default function ForecastCard({ current, day30, day60, seasonEnded, loading }: Props) {
  if (loading) {
    return (
  ...
  const chartData = [
    ...
    const milestones = seasonEnded
    ? [
        { label: 'Final Total', value: current, color: 'text-emerald-400' },
      ]
    : [
    ...
    <div className="card p-5 animate-slide-up" style={{ animationDelay: '250ms' }}>
      <div className="flex items-center gap-2 mb-4">
        <div className="w-7 h-7 rounded-lg bg-emerald-500/20 flex items-center justify-center">
          ...
          {seasonEnded ? 'Season 1 — Final Results' : 'Drizzlet Forecast'}
        </h3>
        <span className={clsx(
          'ml-auto text-[10px] px-2 py-0.5 rounded-full',
          seasonEnded ? 'text-emerald-400 bg-emerald-500/10' : 'text-ika-muted bg-white/5'
        )}>
          {seasonEnded ? 'Ended · Aug 24, 2026' : 'values in billions'}
        </span>
      </div>
      ...
      <ResponsiveContainer width="100%" height={140}>
        <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
          ...
          />
            ...
            <div className={clsx('grid gap-2 mt-3', seasonEnded ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-4')}>
        {milestones.map((m) => (
